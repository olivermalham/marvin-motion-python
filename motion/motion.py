...
def target_velocity(d: int, d_target: int, v_prop: float) -> float:
    ...
    only value measured by the hardware. Assumes that all moves will be at constant acceleration up to maximum velocity.
    V_prop scales maximum velocity and acceleration to permit co-ordinated moves for all wheels together
    ...
    if d < d_ramp:
        ...
        # Deceleration phase
        return v_max * (1 - (d - (d_target - d_ramp)) / d_ramp)  # FIXME?
    else:
        return 0.0
